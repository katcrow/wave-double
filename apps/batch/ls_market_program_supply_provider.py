...
def _parse_response(data: Any, market: str) -> MarketProgramSupplyBar:
    ...
    if not isinstance(rows, list) or len(rows) != 3:
        raise RuntimeError("LS t1631 response malformed: expected three aggregate rows")
    ...
    try:
        ...
        total_indexes = [
            ...
            if volume == sum(other for other_index, other in enumerate(volumes) if other_index != index)
        ]
        ...
        return MarketProgramSupplyBar(market, volumes[total_indexes[0]])
