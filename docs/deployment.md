...
| `/login` 접속 | `200`, 이메일/비밀번호 로그인 폼(이메일 입력 + 비밀번호 입력 + "로그인" 버튼) 렌더 |
| 시크릿 헤더 없이 `/api/dispatch/worker` 호출 | `401` (route가 살아있고 게이트가 걸림을 증명) |

이 smoke test는 인증 없이 확인 가능한 계약만 검증한다. 실제 데이터 렌더링 확인은 아래 ⑥을
따른다.

## ⑥ 슈퍼유저 계정 생성 & 인증 필요 화면 수동 확인

이 앱은 회원가입 플로우가 없다(단일 운영자 전제, 2026-09-02 인증 방식 전환 —
`_bmad-output/planning-artifacts/sprint-change-proposal-2026-09-02.md` 참고).
Neo의 계정은 Supabase 대시보드에서 직접 만들어야 하며, 이 절차는 실제 계정 생성이 필요해
자동화할 수 없다. 배포 후 다음을 수행한다:

1. Supabase 대시보드 > Authentication > Settings에서 **"Allow new users to sign up"을
   비활성화**한다 -- 이 리포는 public이라 공개 회원가입을 열어두면 누구나 계정을 만들 수
   있다.
2. Supabase 대시보드 > Authentication > Users > "Add user"로 Neo의 이메일/비밀번호 계정을
   직접 생성한다("Auto Confirm User"를 켜서 이메일 인증 절차 없이 바로 로그인 가능하게 한다).
3. 그 이메일이 `OPERATOR_ALLOWLIST`(③에서 등록한 환경변수)에 포함돼 있는지 확인한다 --
   로그인 자체는 되더라도 이 allowlist에 없으면 dispatch 권한이 거부된다(AD-7).
4. 실제 배포 URL의 `/login`에서 방금 만든 이메일/비밀번호로 로그인한다.
5. `/`(오늘의 후보, Story 1.9)와 `/runs`(배치 이력, Story 1.9)가 정상 렌더링되는지 확인한다.
6. 화면에 표시되는 데이터가 Story 1.8 스냅샷 API(장중 snapshot)의 실제 값을 반영하는지
