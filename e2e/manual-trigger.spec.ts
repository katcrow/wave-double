 ...
 * Supabase Auth 매직 링크 발급(이메일 수신 확인)이 있어야 검증할 수 있어 CI에서 자동화할 수
 * 없다. 그 케이스는 spec의 Verification "Manual checks"로 옮겼다: 실제 Supabase 프로젝트에
 * migration을 적용한 뒤 브라우저로 로그인해 버튼이 활성화되는지, `curl`로 `request_manual_dispatch`
 * 를 중복 호출해 replay/409 응답을 확인한다.
 *
 * 여기서는 CI에서 자동 검증 가능한 범위 -- 로그인 폼 자체의 렌더링과 미인증 상태에서 워커
 * 라우트가 `/login`으로 리다이렉트되지 않는지(cron 전용 경로 제외 확인) -- 만 검증한다.
 */

test("로그인 폼이 렌더링되고 이메일 제출 UI를 제공한다", async ({ page }) => {
