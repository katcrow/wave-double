...
- 전략 태그 집합은 Epic 7 이후 A~F로 확장되어도 가변적이어야 한다. 동일 후보에 여러 전략 태그가 있어도 수급 수집은 `candidate_id` 기준으로 한 번만 처리하고, F 단독 태그 후보도 active 태그 기반 수집 대상에서 빠지지 않아야 한다.
...
- `publish_attempt`는 supply stage 성공을 발행 전제조건으로 사용한다. 전략 F outcome/tag migration이 운영 환경에 적용되지 않은 경우에는 F 태그 생성부터 발행까지의 운영 수용이 성립하지 않으므로, 수집 stage의 성공만으로 전체 운영 계약을 충족했다고 보지 않는다.
...
- Epic 7의 전략 확장은 Epic 4.10보다 선행한다. 수집 로직은 전략명을 해석하지 않고 active `candidate_tags`의 distinct 후보를 소비해야 하며, F의 계산·DB 제약·outcome rule 운영 적용 여부는 별도 운영 검증으로 확인한다.
