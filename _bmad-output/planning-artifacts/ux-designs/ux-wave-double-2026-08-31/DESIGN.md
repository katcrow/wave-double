...
- **Candidate summary card** — 종목명·코드, 전략 A/B/C 태그, 수급 힌트, 당일 가격/등락률, 시그널 유지/소멸 상태를 한눈에 보여준다. 카드 전체를 클릭하면 근거 패널이 열리고, 선택된 카드는 `{colors.accent}` 경계선을 쓴다.
...
- **Strategy tag** — A/B/C를 태그로 표시하되 태그마다 다른 원색을 주지 않는다. 다중 태그는 가로로 나열하고 좁은 폭에서 줄바꿈하지 않고 `+N`으로 접는다.
