...
전략 A/B/C를 재현하는지 검증하는 **단일 권위 gate**의 고정 입력이다. `backtest/tests/test_golden_fixture.py`
...
{
  "trading_day": "2026-08-26",
  "strategy_signals": { "A": ["종목코드..."], "B": [...], "C": [...] }
}
```
- 참조 시그널 집합: 각 전략(A/B/C)에 대해 창 `[2026-01-01, trading_day]` 내 신호가
  1회 이상 발생한 종목 코드(오름차순) 목록. 각 전략은 비어 있으면 안 된다(빈 픽스처는
  명시적 실패, AD-5).
- 참조 산출 경로: `combine_strategies.build_signals(frame, 3.0, 3.0, ticker)` 기본
  호출(비엄격)로 `screen_abc.screen()`과 동일한 호출·윈도우 규칙을 재현한다.
- 확정 값: A 23종목, B 32종목, C 12종목.
...
- 재생성은 결정적이며, 완료 시 참조 대비 `compute_abc`의 전략별 Jaccard가 0.9 이상인지
