...
def _run(rpc, tagged_fetcher, calendar_client, supply_provider, supply_repo):
    gateway = RunStateGateway(rpc)
    run_id = uuid4()
    lease_token = uuid4()
    return run_supply_stage(
        gateway, tagged_fetcher, calendar_client, supply_provider, supply_repo,
...
def test_normal_all_candidates_produce_three_rows_confirmed():
    ...
    assert slots["D0"].program_net is None
...
def test_holiday_gap_calendar_mapping_is_exact_not_approximated_by_calendar_days():
    ...
    result = run_supply_stage(
        gateway, fetcher, calendar, provider, repo, uuid4(), 1, uuid4(), monday,
    )
