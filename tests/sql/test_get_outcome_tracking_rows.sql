-- Story 5.11: outcome tracking read RPC 계약 fixture. 모든 fixture 데이터는 rollback으로 되돌린다.
begin;
...
insert into public.candidate_outcome(
  ...
  ('00000000-0000-4000-8000-000000000601', 'ZZ001', 'A', '2098-06-01', 1000, 'TP', '2098-06-03', 2.9, 30, 2, 3.0, 3.0),
  ('00000000-0000-4000-8000-000000000602', 'ZZ002', 'B', '2098-06-02', 1000, 'SL', '2098-06-04', -3.0, 30, 2, 3.0, 3.0),
  ('00000000-0000-4000-8000-000000000603', 'ZZ003', 'C', '2098-06-03', 1000, 'TIMEOUT', '2098-07-01', 0.5, 30, 30, 3.0, 3.0),
  ('00000000-0000-4000-8000-000000000604', 'ZZ004', 'D', '2098-06-04', 1000, 'OPEN', null, null, 20, 0, 3.0, 5.0),
  ('00000000-0000-4000-8000-000000000605', 'ZZ005', 'E', '2098-06-05', 1000, 'SUSPENDED', null, null, 30, 0, 2.0, 5.0),
  ('00000000-0000-4000-8000-000000000606', 'ZZ006', 'F', '2098-06-06', 1000, 'DELISTED', '2098-06-07', null, 999999, 1, 3.0, 4.0),
  ('00000000-0000-4000-8000-000000000607', 'ZZ007', 'A', '2098-06-06', 1000, 'TP', '2098-06-07', 1.1, 30, 1, 3.0, 3.0);
...
begin
  ...
  v_row := v->0;
  if not (v_row ? 'outcome_id' and v_row ? 'ticker' and v_row ? 'strategy' and v_row ? 'entry_date'
  ...
  if exists (
    ...
    where (select array_agg(key order by key) from jsonb_object_keys(row) key)
      <> array['entry_date','exit_date','outcome_id','return_pct','status','strategy','ticker']
