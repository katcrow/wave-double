"""전략 H(240이평돌파+120이평우상향필터) 계산 및 재현 백테스트.

원 설계는 docs/240이평돌파_120이평우상향필터.md — SMA240 자체를 매일 갱신되는
추적손절로 쓰고 2%/4% 분할익절(익절우선)이었으나, 프로덕션 청산 규약에 맞춰
단일청산으로 근사했다: TP 4% / SL 5%(고정, 진입가 기준) / 무제한 보유 / 익절우선.
근사 백테스트(baseline 관측창 2020-08-03~2026-08-27): 시그널 770, 거래 591건
(352승/239패), 승률 59.6%, 평균 +0.268%, PF 1.13.

진입 조건 (모두 AND):
1. 오늘 종가가 240일 이동평균(SMA240)을 상향 돌파하는 양봉(종가>시가)
2. 120이평 우상향 필터: 오늘 SMA120 > 어제 SMA120
"""
...
class StrategyHParams:
    ...
    ma_window: int = 240
    slope_ma_window: int = 120
    take_profit_pct: float = 4.0
    stop_loss_pct: float = 5.0
    tp_first: bool = True  # 같은 봉 TP·SL 동시 도달 시 익절 우선
    max_holding_bars: int | None = None  # None = 무제한 보유
...
STRATEGY_H_PARAMS = StrategyHParams()
...
def _validate_params(params: StrategyHParams) -> None:
    if not isinstance(params, StrategyHParams):
        raise ValueError("params는 StrategyHParams여야 합니다")
    for name in ("ma_window", "slope_ma_window"):
...
def validate_strategy_h_params(params: StrategyHParams) -> None:
    """전략 H 파라미터 계약을 외부 검증 코드에서도 사용할 수 있게 노출한다."""
...
def compute_strategy_h(
    frame: pd.DataFrame,
    params: StrategyHParams = STRATEGY_H_PARAMS,
) -> pd.Series:
    """240이평 양봉돌파 + 120이평 우상향 진입 마스크."""
    ...
    return (cross_up & bullish & slope_up).fillna(False).astype(bool)


def strategy_h_signals(
    frame: pd.DataFrame,
    ticker: str = "",
    params: StrategyHParams = STRATEGY_H_PARAMS,
) -> list[SimpleSignal]:
    """전략 H 마스크를 종가 진입용 고정 TP/SL 신호로 변환한다."""

    mask = compute_strategy_h(frame, params)
    return [
        SimpleSignal(
            ticker=ticker,
...
def run_strategy_h_backtest(
    ...
    for ticker in sorted(data):
        ...
        for segment in _valid_segments(frame, valid_rows):
            signals = strategy_h_signals(segment, ticker=ticker, params=params)
            observed_signals = [
...
def _format_result(result: dict[str, object]) -> str:
    return (
        f"전략 H | 시그널 {result['n_signals']} | 거래 {result['n_trades']} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="전략 H(240이평돌파+120이평우상향필터) baseline 백테스트"
    )
    parser.add_argument(
        "--output", type=Path,
        default=RESULTS_DIR / "strategy_h_baseline.csv",
        help="요약 CSV 출력 경로",
    )
    ...
    result = run_strategy_h_backtest(start=args.start, end=args.end)
    print("=== 전략 H(240이평돌파+120이평우상향필터) ===")
