...
from ..indicators import adx, obv, sma
...
STRATEGY_E_PARAMS = StrategyEParams()
...
def _validate_params(params: StrategyEParams) -> None:
    if not isinstance(params, StrategyEParams):
        raise ValueError("params는 StrategyEParams여야 합니다")
    for name in (
        "sma_short_window", "sma_long_window", "obv_window",
        "adx_window", "max_holding_bars",
    ):
        _validate_positive_integer(name, getattr(params, name))
    if params.sma_short_window >= params.sma_long_window:
        raise ValueError("sma_short_window은 sma_long_window보다 작아야 합니다")

    adx_threshold = _validate_finite_real("adx_threshold", params.adx_threshold)
    if not 0.0 <= adx_threshold <= 100.0:
        raise ValueError("adx_threshold은 0~100 범위여야 합니다")
...
def validate_strategy_e_params(params: StrategyEParams) -> None:
    """전략 E 파라미터 계약을 외부 검증 코드에서도 사용할 수 있게 노출한다."""
...
def compute_strategy_e(
    frame: pd.DataFrame,
    params: StrategyEParams = STRATEGY_E_PARAMS,
) -> pd.Series:
    """SMA·OBV·ADX 세 조건의 전략 E 진입 마스크를 계산한다."""

    _validate_frame(frame)
    _validate_params(params)
    close = frame["Close"]

    short_sma = sma(close, params.sma_short_window)
    long_sma = sma(close, params.sma_long_window)
    sma_gc = (short_sma > long_sma) & (
        short_sma.shift(1) <= long_sma.shift(1)
    )

    obv_series = obv(close, frame["Volume"])
    obv_ma = sma(obv_series, params.obv_window)
    obv_gc = (obv_series > obv_ma) & (
        obv_series.shift(1) <= obv_ma.shift(1)
    )

    adx_above_threshold = adx(
        frame["High"], frame["Low"], close, window=params.adx_window
    ) >= params.adx_threshold

    return (sma_gc & obv_gc & adx_above_threshold).fillna(False).astype(bool)
...
def strategy_e_signals(
    ...
    """전략 E 마스크를 종가 진입용 고정 TP/SL 신호로 변환한다."""

    mask = compute_strategy_e(frame, params)
    return [
        SimpleSignal(
            ticker=ticker,
...
def run_strategy_e_backtest(
    ...
    for ticker in sorted(data):
        ...
        for segment in _valid_segments(frame, valid_rows):
            signals = strategy_e_signals(segment, ticker=ticker, params=params)
            observed_signals = [
                signal
...
def _format_result(result: dict[str, object]) -> str:
    return (
        f"전략 E | 시그널 {result['n_signals']} | 거래 {result['n_trades']} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="전략 E(익절2%·고정 SL5%) baseline 백테스트"
    )
    parser.add_argument(
        "--output", type=Path,
        default=RESULTS_DIR / "strategy_e_baseline.csv",
        help="요약 CSV 출력 경로",
    )
    ...
    result = run_strategy_e_backtest(start=args.start, end=args.end)
    print("=== 전략 E(익절2%·고정 SL5%) ===")
