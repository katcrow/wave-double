-- Story 6.3 review patch: the constraint expansion is already applied on
-- environments that recorded 202609051400. Keep the operational comments in
-- sync and fail closed if the expected expanded constraint is absent.
begin;
...
begin
  if not exists (
    ...
    raise exception 'candidate_tags_strategy_check expanded constraint is missing';
  end if;
end $$;
