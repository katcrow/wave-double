-- 2026-09-21: 배치 전략 변경(Neo 확인). 평일 KST 08:10 시작, 1시간 간격으로
-- 19:10까지 intraday(12회), 19:40에 최종 close 1회로 슬롯을 옮긴다(기존
-- 08:30 시작/60분 간격/19:30 close 대비 40분 앞당김). 이 migration은
-- 202609161000_switch_intraday_slots_to_60_minutes.sql의 enqueue_scheduled_dispatch()를
-- 그대로 복사하고 슬롯 분(minute) 조건만 바꾼다. pg_cron 잡(auto-schedule-dispatch-tick)
-- 자체의 매분 tick 주기는 그대로 둔다 -- 비용을 만드는 건 이 함수 안의 판정이지, 바깥
-- tick 주기가 아니다.
-- .github/workflows/scheduled-batch.yml의 on.schedule: cron도 같은 창으로 함께 바꿨다
-- (이중 안전망, 두 경로 모두 logical_run_key fence로 dedupe됨).
begin;

-- 0) logical_runs.logical_run_key_shape CHECK을 :00/:20/:30/:40 -> :00/:10/:20/:30/:40으로
-- 확장한다(과거 이력은 그대로, 신규 슬롯 :10을 허용).
do $$
begin
  if not exists (
    select 1
    ...
    where n.nspname = 'public'
      ...
      and c.contype = 'c'
      and pg_get_constraintdef(c.oid) like '%(00|20|30|40)%'
      and pg_get_constraintdef(c.oid) not like '%10%'
  ) then
...
alter table public.logical_runs
  ...
  check (
    (batch_kind = 'intraday' and logical_run_key ~ '^intraday:[0-9]{4}-[0-9]{2}-[0-9]{2}:([01][0-9]|2[0-3]):(00|10|20|30|40)$')
...
comment on constraint logical_run_key_shape on public.logical_runs is
  '2026-09-21: intraday 슬롯 분(minute)에 :10(신규 스케줄, KST 08:10 시작)을 추가로 허용한다. :00/:20/:30/:40은 이전 스케줄들의 과거 이력과의 호환을 위해 남긴다.';
...
begin
  ...
  -- KST 08:10~19:10 60분 간격(매시 10분) intraday, KST 19:40 close.
  -- (scheduled-batch.yml의 on.schedule: cron 세 항목과 동일한 창.)
  if hh = 19 and mi = 40 then
    batch_kind := 'close';
    logical_key := 'close:' || trading_day::text;
  elsif hh between 8 and 19 and mi = 10 then
    batch_kind := 'intraday';
...
comment on function public.enqueue_scheduled_dispatch() is
  '2026-09-21: KST 08:10 시작, 매시 10분 간격 슬롯(Neo 확인). intraday는 KST 08:10~19:10, close는 KST 19:40 하나뿐이다.';

-- request_manual_dispatch/start_attempt도 같은 정규식으로 logical_run_key 모양을 인라인
-- 검증한다(테이블 CHECK만으로는 두 함수가 예외를 던지기 전에 먼저 막지 못함). 각각의
-- 현재 운영 본문(사전 조회로 확인함)을 그대로 복사하고 정규식 한 줄만 :10까지 확장한다.
...
begin
  ...
  if (p_batch_kind = 'intraday' and p_logical_run_key !~ '^intraday:[0-9]{4}-[0-9]{2}-[0-9]{2}:([01][0-9]|2[0-3]):(00|10|20|30|40)$')
