-- Epic 4 acceptance remediation follow-up: apply after the original
-- candidate-evidence migrations so their older function body cannot replace
-- the published/current-complete and adjusted-price read boundary.
begin;
          ...
          (
            ...
            from (
              select distinct on (s.slot)
                ...
                s.volume,
                coalesce(
                  round(((adjusted_bar.close - previous_bar.close) / nullif(previous_bar.close, 0)) * 100, 2),
                  s.change_pct
                ) as change_pct,
                s.foreign_net,
                s.institution_net,
...
comment on function public.get_candidate_evidence(uuid) is
  'Epic 4 remediation: published current complete supply-success attempt만 읽으며, 후보 trading_day 범위와 adjusted daily_ohlcv 종가를 적용한다.';
